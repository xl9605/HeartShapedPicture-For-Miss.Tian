...
# 白底图片所在的路径
whiteImagePath = os.path.join(dir , 'white', 'white.jpg')
# 白底图片所在的目录
whiteGoalPath = os.path.join(dir, '', 'white')
# 拼接后的图片的总尺寸，根据尺寸和图片数量计算出每张图片的大小
totalSize = 5000
# 重设大小后的图片所在的目录
transferDir = os.path.join(dir, '', 'heartsavereset')
# 拼接结果图片所在的目录
...
# 创建一张分辨率为1920X1080的三通道的白色背景照片
def createBlackImg():
    # 创建一个1920x1080的图片,通道为3
    img = np.zeros([1080,1920,3])
    # 将三个通道都设置为255,即白色
    img[:,:,0] = np.zeros([1080,1920]) + 255
    img[:,:,1] = np.zeros([1080,1920]) + 255
    img[:,:,2] = np.zeros([1080,1920]) + 255
    # 显示该图
...
# 将图片转化为指定大小
def transferSize(allPicPath, height, width):
...
def main(dir):
    ...
    transferSize(allPicPath, height, width)
    # 获取所有格式化后的拼接图片的路径
    ...
    savefileName = resultDir + '/田梦雨的心形照片.PNG'
    toImage.save(savefileName)
    print('制作的心形照片已保存!!保存位置:' + '\n\t' + savefileName)
    print('保存的文件名是:' + os.path.split(savefileName)[-1])
    cv2.imread(savefileName)
    savefileName = resultDir + '/田梦雨的心形照片.JPG'
    toImage = toImage.convert('RGB')
    toImage.save(savefileName)
    print('制作的心形照片已保存!!保存位置:' + '\n\t' + savefileName)
    print('保存的文件名是:' + os.path.split(savefileName)[-1])
    # img = cv2.imread(savefileName)
    # cv2.imshow(os.path.split(savefileName)[-1] , img)
    # cv2.waitKey(0)
